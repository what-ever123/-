<template>
  <div class="class-detail">
    <!-- 顶部返回栏 -->
    <div class="back">
      <div class="back-title" @click="back"><span class="arrow">◂</span><span class="title">{{arr[num]}}</span></div>
      <div>
    ...
    <!-- 书籍 -->
    <ul class="news thebooks-col" style="padding-top:1.3rem;">
        ...
        <div class="book-cell">
          ...
          <div class="tag">
            <div class="tag-left">
              ...
              <span class="author">{{item.author.length>4?item.author.slice(0,4)+'..':item.author}}</span>
            </div>
            <div class="tag-right">
...
export default {
  data(){
    return {
      books:[],
      arr:['现代言情','科幻空间','古代言情','玄幻言情','游戏竞技','仙侠奇缘'],
      arr2:['yanqing','kehuan','gudai','xuanhuan','youxi','xianxia'],
      num:null
    }
  },
  mounted(){
    ...
    this.axios.get('http://localhost:3000/'+this.arr2[this.$route.query.id]).then(res=> {
